...
do
  ...
  if [ $verity -eq 1 ];then
    ...
    if [ "$vm_exist" = "$vm_name" ]; then
       ...
       tmp=$(cat true_test.txt | grep 'passed') 
       fix=$(cat test.txt | grep 'passed')
       if [ "$tmp" = "$fix" ]; then 
          echo "test passed"
          exclude_feature "$j" features_list
       else
          echo "tests echoués"
          app_feature[$app_feat]=${features_list[j]}
          let app_feat+=1
       fi
    else
       echo "la vm n'a pas été crée pour le feature ${features_list[j]}"
...
do
  echo "${app_feature[lo]}" >> app_useful_features.txt
done

for (( io=0 ; io < ${#vm_feature[@]} ; io++ ))
do
  echo "${vm_feature[io]}" >> vm_useful_features.txt
done
